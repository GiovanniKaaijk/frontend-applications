  ...
  getData = place => {
    ...
    const runQuery = (url, query) => {
      ...
      for(let i=0; i<result.length; i++){
        json.results.bindings[i].year.value = parseInt(json.results.bindings[i].year.value);
      ...
      <Router>
        <div className="App">
          ...
          <Route exact path="/" render={props => (
            <div className="home">
              <SearchPlace place={this.searchPlace} history={this.redirect} />
            </div>
          )} />
          <Route path="/overview" render={props => (
            <div className="overview">
              <Render render={this.state.render} toggleLiked={this.toggleLiked} />
              <div className="timeline" >
                <Objects objects={this.state.data} changeData={this.changeData} />
              </div>
            </div>
          )} />
        </div>
